...
USQEXSEADSoundBank* USQEXSEADFactory::FindSEADMusic(FName FileName, FString PackagePath)
{
	...
	if (FileData == nullptr) {
		...
		IFileManager::Get().FindFilesRecursive(AssetFiles, *(FGenericPlatformMisc::ProjectDir() + Content), *AssetFileName, true, false);
...
USQEXSEADSoundBank* USQEXSEADFactory::FindSEADSound(FName FileName, FString PackagePath)
{
	FString FileNameString = FileName.ToString();
	USQEXSEADSoundBank* FileData = (USQEXSEADSoundBank*)StaticFindObject(USQEXSEADSoundBank::StaticClass(), ANY_PACKAGE, *FileNameString);
	if ((FileData == nullptr) && (!PackagePath.IsEmpty())) {
...
UObject* USQEXSEADFactory::FactoryCreateBinary(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const uint8*& Buffer, const uint8* BufferEnd, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	if (FileInfo.ReadAudioData(Buffer, BufferEnd, TinyRawData, TinyRawData.Num(), &ErrorMessage))
	{
		if (FileInfo.bIsSab)
		{
			USQEXSEADSoundBank* SoundObject = FindSEADSound(*Name.ToString(), PackagePath);
			if (SoundObject == nullptr)
			{
				SoundObject = NewObject<USQEXSEADSoundBank>(InParent, Name, Flags);
			}
		...
		{
			USQEXSEADSoundBank* MusicObject = FindSEADMusic(*Name.ToString(), PackagePath);
			if (MusicObject == nullptr)
			{
				MusicObject = NewObject<USQEXSEADSoundBank>(InParent, Name, Flags);
			}
